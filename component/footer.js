let footer = () => {
    return `    <div id="footer_row1">
      <ul class="row1">
        <li>
          <a href="./AboutUs.html"
            >About Us</a
          >
        </li>
        <li>
          <a href="https://www.urbancompany.com/impact" target="_blank"
            >UC Impact</a
          >
        </li>
        <li>
          <a href="https://www.urbancompany.com/terms" target="_blank"
            >Terms &amp; Conditions</a
          >
        </li>
        <li>
          <a href="https://www.urbancompany.com/privacy-policy" target="_blank"
            >Privacy Policy</a
          >
        </li>
        <li>
          <a href="https://www.urbancompany.com/privacy-policy" target="_blank"
            >Interest-Based Advertising</a
          >
        </li>
        <li>
          <a
            href="https://www.urbancompany.com/anti-discrimination-policy"
            target="_blank"
            >Anti Discrimination Policy</a
          >
        </li>
        <li>
          <a href="https://www.urbancompany.com/blog" target="_blank">Blog</a>
        </li>
        <li>
          <a href="https://www.urbancompany.com/reviews" target="_blank"
            >Reviews</a
          >
        </li>
        <li>
          <a href="https://www.urbancompany.com/near-me" target="_blank"
            >Near Me</a
          >
        </li>
        <li>
          <a href="https://careers.urbanclap.com/" target="_blank">Careers</a>
        </li>
        <li>
          <a
            href="https://www.urbancompany.com/purchase-giftcards"
            target="_blank"
            >Gift Cards</a
          >
        </li>
        <li>
        
          <a href="../Urban Company clone/contact.html"
            >Contact Us</a
          >
        </li>
        <li>
          <a href="https://www.urbancompany.com/quick-links" target="_blank"
            >Quick Links</a
          >
        </li>
      </ul>
    </div>
    <div id="footer_row2">
      <hr />
      <p style="color: white">Serving in</p>
      <ul class="main"></ul>
      <li class="top">
        <p id="cuntryname">AUS</p>
        <ul class="row1">
          <li>
            <a href="https://www.urbancompany.com/melbourne" target="_blank"
              >Melbourne</a
            >
          </li>
          <li>
            <a href="https://www.urbancompany.com/sydney" target="_blank"
              >Sydney</a
            >
          </li>
        </ul>
      </li>
      <li class="top">
        <p id="cuntryname">IND</p>
        <ul class="row1">
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank">Agra</a>
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Ahmedabad</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Alwar</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Amritsar</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Aurangabad</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Bangalore</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Bhopal</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Bhubaneswar</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Chandigarh Tricity</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Chennai</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Coimbatore</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Cuttack</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Dehradun</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Delhi NCR</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Guntur</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Guwahati</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Gwalior</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Hyderabad</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Indore</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Jabalpur</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Jaipur</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Jamshedpur</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Jodhpur</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Kakinada</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Kanpur</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Karnal</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Kochi</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Kolkata</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank">Kota</a>
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Lucknow</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Ludhiana</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Madurai</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Meerut</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Mumbai</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Mysore</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Nagpur</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Nashik</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Panipat</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Patna</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Prayagraj</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank">Pune</a>
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Raipur</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Rajahmundry</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Ranchi</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Rohtak</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Sonipat</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Surat</a
            >
          </li>
          <li>
            <a
              href="./urban/Urban Company clone/service.html"
              target="_blank"
              >Thiruvananthapuram</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Udaipur</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Vadodara</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Varanasi</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Vijayawada</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Visakhapatnam</a
            >
          </li>
          <li>
            <a href="./urban/Urban Company clone/service.html" target="_blank"
              >Warangal</a
            >
          </li>
        </ul>
      </li>
      <li class="top">
        <p id="cuntryname">KSA</p>
        <ul class="row1">
          <li>
            <a href="https://www.urbancompany.com/melbourne" target="_blank"
              >Jeddah</a
            >
          </li>
          <li>
            <a href="https://www.urbancompany.com/sydney" target="_blank"
              >Riyadh</a
            >
          </li>
        </ul>
      </li>
      <li class="top">
        <p id="cuntryname">SGP</p>
        <ul class="row1">
          <li>
            <a href="https://www.urbancompany.com/melbourne" target="_blank"
              >Singapore</a
            >
          </li>
        </ul>
      </li>
      <li class="top">
        <p id="cuntryname">UAE</p>
        <ul class="row1">
          <li>
            <a href="https://www.urbancompany.com/abudhabi" target="_blank"
              >Abu Dhabi</a
            >
          </li>
          <li>
            <a href="https://www.urbancompany.com/dubai" target="_blank"
              >Dubai</a
            >
          </li>
          <li>
            <a href="https://www.urbancompany.com/sharjah" target="_blank"
              >Sharjah</a
            >
          </li>
        </ul>
      </li>
      <li class="top">
        <p id="cuntryname">USA</p>
        <ul class="row1">
          <li>
            <a href="https://www.urbancompany.com/austin" target="_blank"
              >Austin</a
            >
          </li>
        </ul>
      </li>
      <hr />
    </div>
    <div id="social_div">
      <div>
        <div id="go_up">
          <a href="./index.html"
            ><img
              src="https://res.cloudinary.com/urbanclap/image/upload/images/growth/home-screen/1631097450980-d2de38.png"
              alt="urban"
          /></a>
        </div>
        <span style="color: white"
          >© 2014-22 UrbanClap Technologies India Pvt. Ltd.</span
        >
      </div>
      <div id="socialicon">
        <ul class="flex">
          <li class="iconcenter">
            <a href="https://twitter.com/urbancompany_UC" target="blank"
              ><img
                class="size20"
                src="../image/twitter.png"
                alt="twiter"
            /></a>
          </li>
          <li class="iconcenter">
            <a href="https://www.facebook.com/UrbanCompany/" target="blank"
              ><img
                class="size20"
                src="../image/facebook.png"
                alt="facebook"
            /></a>
          </li>
          <li class="iconcenter">
            <a href="https://www.instagram.com/urbancompany/" target="blank"
              ><img
                class="size20"
                src="../image/instagram.png"
                alt="insta"
            /></a>
          </li>
          <li class="iconcenter">
            <a href="https://www.youtube.com/channel/UCjNJe0jgLP257X9LsSuyIpg" target="blank"
              ><img
                class="size20"
                src="../image/youtube.png"
                alt="youtube"
            /></a>
          </li>
          <li class="iconcenter">
            <a href="https://www.linkedin.com/company/urbancompany/" target="blank"
              ><img
                class="size20"
                src="../image/linkedin.png"
                alt="linkedin"
            /></a>
          </li>
          <li class="iconcenter">
            <a href="https://www.pinterest.com/UrbanCompanyOfficial/" target="blank"
              ><img
                class="size20"
                src="../image/pinterest.png"
                alt="pinterest"
            /></a>
          </li>
        </ul>
      </div>
      <div id="download">
        <div>
          <a
            href="https://play.google.com/store/apps/details?id=com.urbanclap.urbanclap&amp;referrer=af_tranid%3DpfOfh-acCM6s5iNxNIcs_A%26pid%3DMobileWeb%26c%3Dhome_app_buttons"
            ><img
              src="https://images.urbanclap.com/image/upload/categories/category_v2/category_7f907eb0.png"
              alt="playstore"
          /></a>
        </div>
        <div>
          <a href="https://apps.apple.com/US/app/id1032480595?mt=8"
            ><img
              src="https://images.urbanclap.com/image/upload/categories/category_v2/category_7f741d10.png"
              alt="applestore"
          /></a>
        </div>
      </div>
    </div>`;
}

export { footer };




